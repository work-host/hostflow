...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "is_recruiter", sa.Boolean(), server_default=sa.text("TRUE"), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "stage",
            postgresql.ENUM(name="candidatestage", create_type=False),
            nullable=False,
        ),
        ...
        sa.ForeignKeyConstraint(
            ["owner_id"],
            ["users.id"],
            name="fk_candidates_owner_id_users",
            ondelete="SET NULL",
        ),
        sa.PrimaryKeyConstraint("id"),
    )
